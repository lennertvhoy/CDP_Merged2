...
class DemoSmokeTest:
    ...
    async def check_postgresql(self) -> bool:
        ...
        try:
            ...
            enriched_count = await conn.fetchval(
                "SELECT COUNT(*) FROM companies WHERE email IS NOT NULL"
            )
            
            # Check for demo data presence
            has_website = await conn.fetchval(
                "SELECT COUNT(*) FROM companies WHERE website IS NOT NULL LIMIT 1"
            )
    ...
    async def check_chatbot(self) -> bool:
        ...
        try:
            ...
            self.add_result("chatbot", "health", True,
                          f"Responsive at {chatbot_url}",
                          {"url": chatbot_url})
            print_check("Health Check", True, f"Responsive at {chatbot_url}")
    ...
    def print_final_report(self) -> None:
        ...
        criteria = [
            ...
            ("Demo data available", self.results.get("demo_data", {}).get("demo_usable", {}).get("passed", False)),
