#!/usr/bin/env python3
"""
Import KBO data to PostgreSQL - FILTERED to real companies only
"""
import asyncio
import csv
import os
...
# Entity types that are actual companies (not branches or establishments)
VALID_ENTITY_TYPES = {'1', '2'}  # 1=Legal entity, 2=Natural person with business

# Valid juridical forms (companies, not branches)
VALID_JURIDICAL_FORMS = {
...
async def import_filtered():
    """Import only actual companies (filter branches/establishments)"""
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Starting filtered import...")
    batch = []
    processed = 0
    filtered = 0
    ...
    with open(f"{KBO_DIR}/enterprise.csv", 'r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(reader):
            entity_type = row.get('TypeOfEnterprise', '')
            juridical_form = row.get('JuridicalForm', '').strip()
            
            # Filter: Only import actual companies
            if entity_type not in VALID_ENTITY_TYPES:
                filtered += 1
                continue
            
            if juridical_form and juridical_form not in VALID_JURIDICAL_FORMS:
                filtered += 1
                continue
            ...
            batch.append((
                ...
                'BE', juridical_form[:50] if juridical_form else None, 'KBO'
            ))
            ...
            if len(batch) >= BATCH_SIZE:
                ...
                now = datetime.now()
                if processed % 5000 == 0 or (now - last_log).seconds >= 60:
                    print(f"[{now.strftime('%H:%M:%S')}] Progress: {processed:,} companies | Filtered: {filtered:,}")
                    sys.stdout.flush()
                    last_log = now
    ...
    print(f"  Imported: {processed:,} companies")
    print(f"  Filtered: {filtered:,} non-company entities")
    print(f"  Total in DB: {final_count:,}")
    
    await conn.close()

if __name__ == "__main__":
    asyncio.run(import_filtered())
