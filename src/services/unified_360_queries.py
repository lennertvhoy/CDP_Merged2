...
class Unified360Service:
    ...
    async def get_company_360_profile(
        self, kbo_number: str | None = None, company_uid: str | None = None
        ...
        async with pool.acquire() as conn:
            if kbo_number:
                row = await conn.fetchrow(
                    "SELECT * FROM unified_company_360 WHERE kbo_number = $1", kbo_number
                )
            elif company_uid:
                row = await conn.fetchrow(
                    "SELECT * FROM unified_company_360 WHERE company_uid = $1", company_uid
                )
            else:
    ...
    async def get_industry_pipeline_summary(
        self, nace_prefix: str | None = None, city: str | None = None, limit: int = 50
    ...
    async def get_geographic_distribution(
        self, min_companies: int = 1, limit: int = 100
    ...
    async def get_company_activity_timeline(
        self, kbo_number: str, limit: int = 50, activity_types: list[str] | None = None
    ) -> list[ActivityRecord]:
        ...
        params: list[Any] = [kbo_number]
        param_idx = 2

        if activity_types:
            placeholders = ",".join(
                f"${i}" for i in range(param_idx, param_idx + len(activity_types))
            )
    ...
    async def search_companies_unified(
        self, query: str, search_fields: list[str] | None = None, limit: int = 20
    ) -> list[dict[str, Any]]:
